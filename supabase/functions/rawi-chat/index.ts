...
type Landmark = {
  "اسم المكان": string; "النوع": string; "المنطقة": string;
  "شرح تفصيلي": string; "لماذا مشهور": string; "الأنشطة": string;
  "مناسب للعوائل": string; "مدة الزيارة المقترحة": string; "أوقات الزيارة": string;
  "أقرب أماكن": string; "رابط رسمي": string; "أسئلة متوقعة": string; "إجابة مختصرة": string;
};
const RIYADH_LANDMARKS_REF = (riyadhLandmarks as Landmark[]).map(l =>
  `• ${l["اسم المكان"]} (${l["النوع"]} — ${l["المنطقة"]})\n  الشرح: ${l["شرح تفصيلي"]}\n  لماذا مشهور: ${l["لماذا مشهور"]}\n  الأنشطة: ${l["الأنشطة"]}\n  مناسب للعوائل: ${l["مناسب للعوائل"]} | المدة: ${l["مدة الزيارة المقترحة"]} | الأوقات: ${l["أوقات الزيارة"]}\n  أقرب أماكن: ${l["أقرب أماكن"]}${l["رابط رسمي"] ? `\n  رابط: ${l["رابط رسمي"]}` : ""}${l["أسئلة متوقعة"] ? `\n  س متوقع: ${l["أسئلة متوقعة"]} — ج: ${l["إجابة مختصرة"]}` : ""}`
).join("\n\n");

const RIYADH_KNOWLEDGE_BLOCK = `\n\nمرجعية موثوقة عن الرياض — استخدمها كأساس في إجاباتك ولا تخترع خارجها:\n\n[كلمات الرياض ومعانيها]\n${RIYADH_WORDS_REF}\n\n[أمثال شعبية]\n${RIYADH_PROVERBS_REF}\n\n[عادات وتراث]\n${RIYADH_CUSTOMS_REF}\n\n[أماكن الرياض]\n${RIYADH_PLACES_REF}\n\n[معالم الرياض — شرح تفصيلي]\n${RIYADH_LANDMARKS_REF}\n\n[أسئلة شائعة وإجاباتها]\n${RIYADH_QA_REF}`;
